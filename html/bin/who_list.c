...
//  title = regreplace(title, "[^A-Za-z0-1\s*]", "", 1);
  level = (int)dude->query_player_level();
  
  title = subs(title,"<","");
  title = subs(title,">","");
  pretitle = subs(pretitle,"<","");
  pretitle = subs(pretitle,">","");

   if (alignment != "")
   {
     dispname = pretitle + " <i><b>" + capitalize(name) + "</b></i> " + title + " ("+ alignment+")";
   }
   else
   {
     dispname = pretitle + " <i><b>" + capitalize(name) + "</b></i> " + title;
   }
...
void main() { 
  ...
  out += sprintf("<B>%d Visible Users Currently on 3 Kingdoms (%s EST)</B></CENTER>\n"
